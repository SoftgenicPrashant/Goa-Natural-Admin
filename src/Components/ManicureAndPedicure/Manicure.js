...
useEffect(() => {
  ...
  const fetchPosts = async () =>{
    const res = await axios.get("http://localhost:5000/api/allManicure");
    SetPost(res.data)
  }
  fetchPosts();
},[]);
...
const deleteHairProduct = async id => {
  await axios.delete(`http://localhost:5000/api/deletemanicureProduct/${id}`)
    ...
    <div>
        <SideBar />
            <Container className='hairs'>
                <h1 className='text-center mt-3'> Manicure and Peticure</h1>
 ...
 <Col md={4} xs={6}>
            ...
            <Link to="/AddManicureProducts" className="float">
